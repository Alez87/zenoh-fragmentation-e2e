...
mod tests_put {
    ...
    async fn path_invalid() {
        let (config, path, value, chunk_size) =
            common::setup_put("peer", "", "image.png", 65_000);
    ...
    async fn file_notfound() {
        let (config, path, value, chunk_size) =
            common::setup_put("peer", "/demo/example/myfile", "wrong_path/image.png", 65_000);
        let result = common::call_put(config, path, value, chunk_size).await;    
...
mod tests_get {
    ...
    async fn path_invalid() {
        ...
        let result = common::call_get(config, selector, root_folder, root_folder_chunks, index_start, index_end, chunk_index_start, chunk_index_end).await;
        println!("Result: {:?}", result);
        assert_eq!(Err(io::ErrorKind::InvalidInput), result);
    }
